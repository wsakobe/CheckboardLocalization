...
private:
    ...
    void buildMatrix(const Mat &img, std::vector<pointInform> &crossPtsList, int cnt, double fps, bool ButtonPressed, bool ZButtonPressed, std::vector<Point3f>& endEffectorWorldPoints);                                                                          // 基于crossPtsList解读棋盘格信息, 改变mLabel和mPos
    ...
    void solveTransformationUsingICP(const Mat& img, std::vector<pointInform>& crossPtsList, std::vector<matrixInform> matrix, int labelnum, std::vector<Point2f>& cartisian_dst, bool update[10], double fps, bool ButtonPressed, bool ZButtonPressed, std::vector<Point3f>& endEffectorWorldPoints);
...
public:
    ...
    void feed(const Mat &img, int cnt, double fps, bool ButtonPressed, bool ZButtonPressed, std::vector<Point3f>& endEffectorWorldPoints);
    // 向交叉点响应器输入图像和测试点
};
